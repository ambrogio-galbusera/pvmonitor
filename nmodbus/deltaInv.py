import crc,struct,sys,binascii
...
class DeltaInverter:
    ...
    def __findCmdInfo(self,cmdValue):
        for k,v in self.cmds.items():
            if(bytes(k,'utf-8')==cmdValue):
                return v
    ...
    def __unpackFormatted(self,cmd):
        ...
        try:
            print("cmd ", self.cmds[cmdcontent[3:5]])
    ...
    def __unpackData(self,cmd):
        ...
        try:
            stringName,fmt,divisor,unit = self.__findCmdInfo(cmdcontents[3:5]) 
            if fmt==0: ##General Numbers
                resp,invNum,size,instruction,value = struct.unpack('>BBB2sH',cmdcontents)
                value = value / divisor
            elif fmt==1: ##ascii string
